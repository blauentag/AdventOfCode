defmodule TreeTopHouse do
  ...
  def hidden_down?(tree_map, row, col) do
    ...
    Enum.reduce_while((row + 1)..height, false, fn r, _acc ->
      if tree_height <= Enum.at(Enum.at(tree_map, r), col),
        do: {:halt, true},
        else: {:cont, false}
    end)
  end
  ...
  def hidden_left?(tree_map, row, col) do
    ...
    Enum.reduce_while((col - 1)..0, false, fn c, _acc ->
      if tree_height > Enum.at(Enum.at(tree_map, row), c), do: {:cont, false}, else: {:halt, true}
    end)
  end
  ...
  def hidden_right?(tree_map, row, col) do
    ...
    Enum.reduce_while((col + 1)..width, false, fn c, _acc ->
      if tree_height <= Enum.at(Enum.at(tree_map, row), c),
        do: {:halt, true},
        else: {:cont, false}
    end)
  end
  ...
  def hidden_up?(tree_map, row, col) do
    ...
    Enum.reduce_while((row - 1)..0, false, fn r, _acc ->
      if tree_height > Enum.at(Enum.at(tree_map, r), col), do: {:cont, false}, else: {:halt, true}
    end)
  end

  def hidden_all_around?(tree_map, row, col) do
    hidden_down?(tree_map, row, col) and hidden_left?(tree_map, row, col) and
      hidden_right?(tree_map, row, col) and hidden_up?(tree_map, row, col)
  end

  def not_visible(tree_map, row, col, acc) do
    cond do
      hidden_all_around?(tree_map, row, col) -> Map.update!(acc, :hidden, fn h -> h + 1 end)
      true -> Map.update!(acc, :visible, fn v -> v + 1 end)
    end
  end
  ...
  def count(tree_map, row, width) do
    ...
    Enum.reduce(0..width, %{hidden: 0, visible: 0}, fn col, acc ->
      cond do
        row == 0 or col == 0 or row == height or col == width ->
          Map.update!(acc, :visible, fn visible -> visible + 1 end)

        true ->
          not_visible(tree_map, row, col, acc)
      end
    end)
  end
  ...
  def trees_down(tree_map, row, col) do
    ...
    Enum.reduce_while((row + 1)..height, 1, fn r, acc ->
      if tree_height > Enum.at(Enum.at(tree_map, r), col) and r != height,
        do: {:cont, acc + 1},
        else: {:halt, acc + 0}
    end)
  end
  ...
  def trees_left(tree_map, row, col) do
    ...
    Enum.reduce_while((col - 1)..0, 1, fn c, acc ->
      if tree_height > Enum.at(Enum.at(tree_map, row), c) and c != 0,
        do: {:cont, acc + 1},
        else: {:halt, acc + 0}
    end)
  end
  ...
  def trees_right(tree_map, row, col) do
    ...
    Enum.reduce_while((col + 1)..width, 1, fn c, acc ->
      if tree_height > Enum.at(Enum.at(tree_map, row), c) and c != width,
        do: {:cont, acc + 1},
        else: {:halt, acc + 0}
    end)
  end
  ...
  def trees_up(tree_map, row, col) do
    ...
    Enum.reduce_while((row - 1)..0, 1, fn r, acc ->
      if tree_height > Enum.at(Enum.at(tree_map, r), col) and r != 0,
        do: {:cont, acc + 1},
        else: {:halt, acc + 0}
    end)
  end

  def scenic_score(tree_map, row, col, acc) do
    Enum.max([
      trees_down(tree_map, row, col) *
        trees_left(tree_map, row, col) *
        trees_right(tree_map, row, col) *
        trees_up(tree_map, row, col),
      acc
    ])
  end
    ...
    Enum.reduce(0..width, 0, fn col, acc ->
      cond do
        row != 0 and col != 0 and row != height and col != width ->
          scenic_score(tree_map, row, col, acc)
